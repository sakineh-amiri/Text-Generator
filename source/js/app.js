...
let $ = document;
let inputParagrafNumber = $.querySelector("#amount");
let resultElem = $.querySelector("#result");
let generateBtn = $.querySelector("#generate");
let languageBtn = $.querySelector("#language");
var flag = true;

//language btn
languageBtn.addEventListener("click", function (e) {
  e.preventDefault();
  if (languageBtn.innerHTML === "فارسی" && flag) {
    languageBtn.innerHTML = "english";
    flag = false;
    // console.log(flag);
  } else if (languageBtn.innerHTML === "english" && !flag) {
    languageBtn.innerHTML = "فارسی";
    flag = true;
    // console.log(flag);
  }
});
...
generateBtn.addEventListener("click", function (e) {
  ...
  if (flag) {
    //persion
    let resultArrayPersion = [];
    if (
      isNaN(inputParagrafNumber.value) ||
      inputParagrafNumber.value > 9 ||
      inputParagrafNumber.value < 0 ||
      inputParagrafNumber.value === ""
    ) {
      inputParagrafNumber.value = 1;
    }
    for (let r = 0; r < inputParagrafNumber.value; r++) {
      resultArrayPersion.push(dummyTextPersion[0]);
    }
    // console.log(resultArrayPersion);
    resultElem.innerHTML = resultArrayPersion.join("<br><br>");
    inputParagrafNumber.value = "";
    inputParagrafNumber.focus();
  } else {
    // english;
    if (
      isNaN(inputParagrafNumber.value) ||
    ...
    // console.log(inputParagrafNumber.value);
    let resultArray = [];
    for (let i = 0; i < inputParagrafNumber.value; i++) {
      resultArray.push(dummyText[Math.floor(Math.random() * dummyText.length)]);
    }
    console.log(resultArray);
    resultElem.innerHTML = resultArray.join("<br><br>");
    inputParagrafNumber.value = "";
    inputParagrafNumber.focus();
  }
});
